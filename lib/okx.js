...
// ─── fetchSince2026: sequential pagination with fallback ─────────────────────
const START_TS = new Date("2026-01-01T00:00:00Z").getTime();

// Batas max halaman per timeframe agar tidak flood rate-limit OKX
const MAX_PAGES = { "15m": 20, "1H": 30, "4H": 40 };
...
async function fetchSince2026({ instId, bar }) {
  ...
  const now = Date.now();
  const totalPages = Math.ceil((now - START_TS) / candleMs / OKX_HISTORY_LIMIT);
  const maxPages   = Math.min(totalPages, MAX_PAGES[bar] ?? 20);

  // Halaman 0 = terbaru, halaman N = semakin lama
  const pagePromises = Array.from({ length: maxPages }, (_, i) => {
    ...
    if (i > 0) {
      const beforeTs = now - i * OKX_HISTORY_LIMIT * candleMs;
      if (beforeTs < START_TS) return null;
      params.set("after", String(beforeTs));
    }
    return fetchOkxJson(`/history-candles?${params.toString()}`)
      .then((batch) => batch.map(normalizeCandle).filter((c) => c.t >= START_TS))
      .catch(() => null); // null = gagal (beda dengan [] yang valid kosong)
  }).filter(Boolean);

  // Sequential dengan CONCURRENCY=3 agar tidak kena rate-limit
  const CONCURRENCY = 3;
  const allRows = [];
  let failCount = 0;
  ...
  for (let i = 0; i < pagePromises.length; i += CONCURRENCY) {
    ...
    for (const rows of results) {
      if (rows === null) { failCount++; }
      else { allRows.push(...rows); }
    }
    // Stop early jika 3 batch berturut-turut gagal semua
    if (failCount >= 3 && allRows.length === 0) break;
    // Jeda kecil antar batch untuk hindari rate-limit
    if (i + CONCURRENCY < pagePromises.length) {
      await new Promise(r => setTimeout(r, 120));
    }
  }

  // Jika tidak dapat data sama sekali → fallback ke recent candles
  if (allRows.length === 0) {
    console.warn(`[fetchSince2026] ${instId}/${bar} — semua page gagal, fallback ke recent`);
    const recent = await fetchRecentCandles({ instId, bar });
    cacheSet(key, recent);
    return recent;
  }
