...
//
//  Implementasi 1:1 dari PineScript v6 resmi TradingView:
//    [supertrend, direction] = ta.supertrend(factor, atrPeriod)
//
//  Konvensi direction SAMA dengan PineScript:
//    direction = -1  → Uptrend   (garis = lowerBand, HIJAU, di BAWAH harga)
//    direction =  1  → Downtrend (garis = upperBand, MERAH, di ATAS harga)
//
//  ATR dihitung manual via True Range + RMA (Wilder's MA) —
//  identik dengan ta.atr() PineScript, bukan library pihak ketiga.
//
//  Juga menghasilkan bodyMiddle[] untuk fill shading (seperti Pine fill()).
// ═══════════════════════════════════════════════════════════════════════════════
export function calcSuperTrend(highs, lows, closes, period = 10, multiplier = 3.0) {
  const n = closes.length;

  // ── Step 1: True Range — identik ta.tr(true) PineScript ──────────────────
  ...
  // ── Step 2: ATR via RMA — identik ta.rma(ta.tr(true), length) ────────────
  // Seed = SMA dari `period` TR pertama, lalu alpha = 1/period (Wilder)
  ...
  // ── Step 3: SuperTrend ────────────────────────────────────────────────────
  // Translasi langsung dari source ta.supertrend() PineScript:
  //
  //   src = hl2
  ...
  //   elif prevST == prevUpper   → direction = close > upperBand ? -1 : 1
  ...
  const bodyMiddle = new Array(n).fill(null); // untuk fill shading di chart

  let prevUpper    = null;
  let prevLower    = null;
  let prevST       = null;
  let prevATRValid = false; // apakah atr[i-1] sudah valid (tidak na)

  for (let i = 0; i < n; i++) {
    bodyMiddle[i] = (highs[i] + lows[i]) / 2; // proxy hl2 (open[] tidak ada)
    ...
    // Band clamping — sama persis logika PineScript
    if (prevLower !== null) {
      const prevClose = closes[i - 1];
      lowerBand = (lowerBand > prevLower || prevClose < prevLower)
        ? lowerBand : prevLower;
    }
    if (prevUpper !== null) {
      const prevClose = closes[i - 1];
      upperBand = (upperBand < prevUpper || prevClose > prevUpper)
        ? upperBand : prevUpper;
    }

    // Direction — translasi langsung dari PineScript
    let dir;
    if (!prevATRValid) {
      // na(atr[1]) — bar pertama yang punya ATR
      dir = 1;
    } else if (prevST === prevUpper) {
      // Sebelumnya downtrend → apakah close menembus upper ke atas?
      dir = closes[i] > upperBand ? -1 : 1;
    } else {
      // Sebelumnya uptrend → apakah close menembus lower ke bawah?
  ...
  // bodyMiddle: akan dipakai di page.jsx untuk fill shading
  return { supertrend, direction, bodyMiddle };
}
